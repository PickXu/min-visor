...
{
	return tlsf_cast(u64, block->size & block_header_free_bit);
}
...
{
	return tlsf_cast(u64, block->size & block_header_prev_free_bit);
}
...
{
	return tlsf_cast(block_header_t*, tlsf_cast(u64, ptr) + size);
}
...
{
	const u64 aligned =
		(tlsf_cast(u64, ptr) + (align - 1)) & ~(align - 1);
	tlsf_assert(0 == (align & (align - 1)) && "must align to a power of two");
...
static void mapping_insert(size_t size, int* fli, int* sli)
{
	...
	if (size < SMALL_BLOCK_SIZE)
	{
		...
		fl = 0;
		sl = tlsf_cast(u64, size) / (SMALL_BLOCK_SIZE / SL_INDEX_COUNT);
	}
	else
	{
		fl = tlsf_fls_sizet(size);
		sl = tlsf_cast(u64, size >> (fl - SL_INDEX_COUNT_LOG2)) ^ (1 << SL_INDEX_COUNT_LOG2);
		fl -= (FL_INDEX_SHIFT - 1);
	}
	*fli = fl;
	*sli = sl;
...
void* tlsf_memalign(tlsf_pool tlsf, size_t align, size_t size)
{
	...
	*/
	...
	if (block)
	{
		...
		size_t gap = tlsf_cast(u64,
			tlsf_cast(u64, aligned) - tlsf_cast(u64, ptr));
